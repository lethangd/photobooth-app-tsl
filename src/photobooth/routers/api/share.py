...
def api_download_item_id_get_sharelinks(id: UUID):  # -> list[str]:
    ...
    shareondemand_link = container.share_ondemand_service.get_share_link(
        identifier=requested_mediaitem.id, filename=requested_mediaitem.processed.name
    )
    ...
    return shareondemand_link + pluggy_links_flatten
